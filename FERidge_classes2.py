...
import numpy as np
import pandas as pd
import cvxpy as cp # ottimizzazione convessa
...
def _build_design(df: pd.DataFrame):
    ...
    baseline_release = release_levels[0]  # Effetti relativi a Apr23
...
class ConstrainedFEScenarioSimulator:
    ...
    def __init__(self, fitted_model: ConstrainedFEModel,training_data: pd.DataFrame, baseline_data: pd.DataFrame):
        """
        ...
        baseline_data : DataFrame, optional
            Baseline Scenario Dataset to use as baseline (if Empty, uses mean of last 4 releases of training_data)
        """
    ...
    def _create_baseline(self):
        ...
        if self.baseline_data.empty:
            ...
            last_releases = unique_releases#[-4:]  # last 4 releases
            ...
            if self.baseline_data.empty:
                raise ValueError(f"Error in creating the baseline scenario")

        self.baseline_years = self.baseline_data['Year']
    ...
    def _calculate_confidence_interval(self, prediction, residual_variance=None, confidence=0.95):
    ...
    def simulate_scenario(self, scenario_dict, use_year_specific_residuals=True):
        ...
        for year, variable_changes in scenario_dict.items():
            ...
            residual_variance = 1.537 if self.y_col == "Baseload_Price_EUR_MWh" else 1.335  # variances of training set residuals
            ci_interval = self._calculate_confidence_interval(scenario_prediction_corrected, residual_variance)
            print ("residual:", baseline_residual)
            print("prediction :",scenario_prediction_raw)
            # Store results
            results[year] = {
                'baseline': {
                    ...
                    'corrected_value': baseline_predicted + baseline_residual,
            ...
            print(f"Model Variable Changes:")
